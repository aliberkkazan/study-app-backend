...
export class RenameProgramToTask1767799999999 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "task" RENAME COLUMN "student_id" TO "owner_id"`,
    );
    await queryRunner.query(
      `ALTER TABLE "task" RENAME COLUMN "mentor_id" TO "assigned_by"`,
    );

    // Add new columns
    await queryRunner.query(
      `ALTER TABLE "task" ADD "subject" character varying`,
    );
    await queryRunner.query(`ALTER TABLE "task" ADD "topic" character varying`);
    await queryRunner.query(
      `ALTER TABLE "task" ADD "source" character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "task" ADD "target_outcome" character varying`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    // Revert column names
    await queryRunner.query(
      `ALTER TABLE "task" RENAME COLUMN "assigned_by" TO "mentor_id"`,
    );
    await queryRunner.query(
      `ALTER TABLE "task" RENAME COLUMN "owner_id" TO "student_id"`,
    );
