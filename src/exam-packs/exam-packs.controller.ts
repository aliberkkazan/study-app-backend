...
export class ExamPacksController {
  ...
  @ApiOperation({
    summary:
      'List all available exams and versions, optionally filtered by country',
  })
  @ApiQuery({
    name: 'countryCode',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description: 'List of exams with country and versions',
    type: [Exam],
  })
  async getAllExamPacks(
  ...
  @Get('yks/current')
  @ApiOperation({
    summary: 'Get current official YKS curriculum and hierarchy',
  })
  @ApiResponse({
    status: 200,
    description: 'Current YKS version details and topic hierarchy',
    type: ExamVersion,
  })
  ...
  @Get('exams/:id')
  @ApiOperation({
    summary: 'Get exam by ID with all versions, sections and subjects',
  })
  @ApiResponse({
    status: 200,
    description: 'Exam details with sections',
    type: Exam,
  })
  ...
  @ApiOperation({
    summary: 'Get full section-subject-topic hierarchy for an exam version',
  })
  @ApiResponse({
    status: 200,
    description: 'Hierarchy tree for mobile rendering',
    type: ExamVersion,
  })
  async getVersionHierarchy(
